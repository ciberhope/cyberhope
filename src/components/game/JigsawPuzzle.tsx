...
export interface JigsawPuzzleProps {
  ...
  /** Numero de tiles que aparecerao na tela */
  tilesDisponiveis?: number;
...
export const JigsawPuzzle: FC<JigsawPuzzleProps> = ({
  ...
  const onImageLoaded = useCallback(
    (image: HTMLImageElement) => {
      ...
      const arrayBuffer = Array.from(Array(rows * columns).keys()).map(
        (position) => ({
      ...
      shuffleArray(arrayBuffer);
      setTiles(arrayBuffer);
    },
    [rows, columns]
  );
  ...
  const arrayLimitadoPorPerguntas =
    tiles && tiles.slice(Math.max(tiles.length - tilesDisponiveis, 0));
  ...
  return (
    ...
    >
      {arrayLimitadoPorPerguntas &&
        rootSize &&
        imageSize &&
        arrayLimitadoPorPerguntas.map((tile) => (
          <div
            draggable={false}
